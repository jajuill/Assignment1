...
public class ItemList {
	...
	private void changeExpense(double value, String curr){//checks to see if there is another expense object of the same Currency, and if there is, adds/removes value from currency total
		for(int i=0;i<=expenseList.size();i++){//         //if currency object does not exist, makes new one
			if (i == expenseList.size()){//then must not have currency yet because been through all
				...
				break;//probably could use variable to avoid uing break, but meh. Works pretty damn well
			}
		}
	}
